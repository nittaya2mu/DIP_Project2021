...
Dprevious=0;
iter=0;
for i=1:c, Covainv(:,:,i)=eye(N); end % initialize covariance matrix

while converged==0,
   iter=iter+1;

   % step A. evaluation of distortion using dtype norm
   tmp=dist(X,W,dtype,Covainv);    % K x C
   ...
   if dtype==0, % L2 norm or weighted L2 norm
      Dpresent=sum(tmp1.*tmp1);    % distortion before weight is adjusted.
   elseif dtype==1, % L1 norm
      Dpresent=sum(tmp1);
   elseif dtype==2, % L_Inf norm
      Dpresent=max(tmp1);
   elseif dtype==3, % L2 norm or weighted L2 norm
      Dpresent=sum(tmp1.*tmp1);    % distortion before weight is adjusted.
      for i=1:c, % compute covariance matrix
         ...
         if nj(i) > 0,
            ...
            [v,d]=eig(Cova(:,:,i));
            d=diag(d); % change to N x 1 vector
            if min(d)<= 1e-3,
               d=max(d,1e-3);
            end
            Covainv(:,:,i)=v*inv(diag(d))*v';
         else
            Covainv(:,:,i)=eye(N);
         end % if nj(i)
      end % for i
...
for i=1:c,                 % update code words
   ...
   if nj(i) > 0,
      ...
      disp([int2str(i) 'th cluster is empty!'])
   end   
